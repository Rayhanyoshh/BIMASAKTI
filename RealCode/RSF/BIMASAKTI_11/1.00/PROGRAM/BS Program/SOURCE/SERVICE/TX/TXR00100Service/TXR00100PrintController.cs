...
using TXR00100Common.DTOs.PrintDTO;
using R_BackEnd;
using R_Cache;
using R_Common;
...
public class TXR00100PrintController: R_ReportControllerBase
{
    private LoggerTXR00100Print _logger;
    private R_ReportFastReportBackClass _ReportCls;
    private PrintParamTXDTO _Parameter;
    private readonly ActivitySource _activitySource;
    
    #region instantiation

    public TXR00100PrintController(ILogger<LoggerTXR00100Print> logger)
    {
        LoggerTXR00100Print.R_InitializeLogger(logger);
        _logger = LoggerTXR00100Print.R_GetInstanceLogger();
        _activitySource = TXR00100Activity.R_InitializeAndGetActivitySource(nameof(TXR00100PrintController));
    ...
    {
        pcFileTemplate = System.IO.Path.Combine("Reports", "TXR00100.frx");
    }
    ...
    [HttpPost]
    public R_DownloadFileResultDTO DownloadResultPrintPost (PrintParamTXDTO poParameter)
    {
        ...
        PrintLogKeyTXDTO loCache = null;
        R_DownloadFileResultDTO loRtn = null;
        try
        {
            loRtn = new R_DownloadFileResultDTO();
            loCache = new PrintLogKeyTXDTO
            {
                poParam = poParameter,
    ...
    {
        using Activity activity = _activitySource.StartActivity("Report");
        R_Exception loException = new R_Exception();
        FileStreamResult loRtn = null;
        PrintLogKeyTXDTO loResultGUID = null;
        try
        {
            // Deserialize the GUID from the cache
            loResultGUID = R_NetCoreUtility.R_DeserializeObjectFromByte<PrintLogKeyTXDTO>(
    ...
    private TXR00100PrintResultWithBaseHeaderPrintDTO GenerateDataPrint(PrintParamTXDTO poParam)
    {
        ...
        try
        {
            ...
            TXR00100PrintColoumnDTO loColumnObject = new TXR00100PrintColoumnDTO();
            var loColumn = AssignValuesWithMessages(typeof(TRX00100BackResources.Resources_Dummy_Data),
                loCultureInfo, loColumnObject);

            // Set base header data
            _logger.LogDebug("Deserialized Print Parameters: {@PrintParameters}");
            var loCls = new TXR00100Cls();

            loParam.CCOMPANY_NAME = R_BackGlobalVar.COMPANY_ID.ToUpper();
            loParam.CPRINT_CODE = "001";
            loParam.CPRINT_NAME = "Withholding Tax Slip Report";
            ...
            PrintResultTXDTO loData = new PrintResultTXDTO()
            {
                ...
                Header = "Report",
                Column = (TXR00100PrintColoumnDTO)loColumn,
                Data = new List<TXR00100DataResultDTO>(),
                Param = poParam
            };
